<%-- 
    Document   : bukuSaya
    Created on : 2 Jun 2025, 19.35.03
    Author     : alif
--%>
    ...
    <body>
        ...
        <div class="buku-saya">
            ...
            <%
                try {
                    JDBC db = new JDBC();
                    ResultSet rsBuku = db.getDataAll("SELECT * FROM buku");
                    ResultSet rsBukuSaya = db.getDataAll("SELECT DISTINCT judul_buku FROM pembelian WHERE nama_pembeli = '" + username + "'");

                    Set<String> bukuSayaSet = new HashSet<>();
                    while (rsBukuSaya != null && rsBukuSaya.next()) {
                        bukuSayaSet.add(rsBukuSaya.getString("judul_buku"));
                    }
                    rsBukuSaya.close();

                    while (rsBuku != null && rsBuku.next()) {
                        String idBuku = rsBuku.getString("id_buku");
                        String penulis = rsBuku.getString("penulis");
                        String judul = rsBuku.getString("judul");
                        String harga = rsBuku.getString("harga");
                        String gambar = rsBuku.getString("gambar");

                        if (bukuSayaSet.contains(judul)) {
            %>
            <div class="book-item">
                <button class="wishlist-btn active" data-id="<%= idBuku%>" onclick="toggleWishlist(event, this)">
                    <i class="fas fa-heart"></i>
                </button>
                <a href="https://telkomuniversityofficial-my.sharepoint.com/personal/informaticslab_telkomuniversity_ac_id/_layouts/15/onedrive.aspx?ga=1&id=%2Fpersonal%2Finformaticslab%5Ftelkomuniversity%5Fac%5Fid%2FDocuments%2FIFLAB%20PERIODE%2024%2D25%2FGENAP%2FPRAKTIKUM%2FMODUL%2FDKA%2FMODUL%20DKA%202425%2D2%2Epdf&parent=%2Fpersonal%2Finformaticslab%5Ftelkomuniversity%5Fac%5Fid%2FDocuments%2FIFLAB%20PERIODE%2024%2D25%2FGENAP%2FPRAKTIKUM%2FMODUL%2FDKA" target="_blank">
                    <img src="<%= gambar%>" alt="<%= judul%>">
                    <p id="penulis"><%= penulis%></p>
                    <p id="judul"><%= judul%></p>
                    <p id="harga">Rp<%= new DecimalFormat("#,##0").format(Float.parseFloat(harga))%></p>
                </a>
            </div>
            <%
                        }
        ...
        <script>
            ...
            const booksContainer = document.querySelector('.buku-saya');
